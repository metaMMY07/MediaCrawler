...
//
// This file is part of the MediaCrawler project and is distributed under
// NON-COMMERCIAL LEARNING LICENSE 1.1.  It intentionally contains no Node.js
// dependency: Android evaluates it in a WebView JavaScript runtime.

var init_str = "6fpLRqJO8M/c3jnYxFkUVC4ZIG12SiH=5v0mXDazWBTsuw7QetbKdoPyAl+hN9rgE";
var h = {
...
function G(e) {
    var t = new Array(4), n = new Array(4);
    i(e, t, 0);
    n[0] = h.zb[255 & t[0]];
    n[1] = h.zb[255 & t[1]];
    n[2] = h.zb[255 & t[2]];
...
function array_0_16_offset(e) {
    var t = new Array(16), n = new Array(36);
    n[0] = B(e, 0);
    n[1] = B(e, 4);
    n[2] = B(e, 8);
    n[3] = B(e, 12);
...
function array_16_48_offset(e, t) {
    for (var n = [], r = e.length, index = 0; r > 0; r -= 16) {
        var o = e.slice(16 * index, 16 * (index + 1)), a = new Array(16);
        for (var c = 0; c < 16; c++) a[c] = o[c] ^ t[c];
...
function encode_0_16(array_0_16) {
    var result = [], array_offset = [48, 53, 57, 48, 53, 51, 102, 55, 100, 49, 53, 101, 48, 49, 100, 55];
    for (var index = 0; index < array_0_16.length; index++) {
        var a = array_0_16[index] ^ array_offset[index], b = a ^ 42;
        result.push(b);
    }
...
function encode(ar) {
    var b = ar[1] << 8, c = ar[0] | b, d = ar[2] << 16, e = c | d, result_array = [], x6 = 6;
...
function get_init_array(encode_md5) {
    var init_array = [];
    for (var index = 0; index < encode_md5.length; index++) init_array.push(encode_md5.charCodeAt(index));
    init_array.unshift(0);
    init_array.unshift(Math.floor(Math.random() * 127));
    while (init_array.length < 48) init_array.push(14);
    var array_0_16 = encode_0_16(init_array.slice(0, 16));
    var array_16_48 = array_16_48_offset(init_array.slice(16, 48), array_0_16);
    return array_0_16.concat(array_16_48);
}

function get_zse_96(encode_md5) {
    var result_array = [], init_array = get_init_array(encode_md5), result = "";
    for (var index = 47; index >= 0; index -= 4) init_array[index] ^= 58;
    init_array.reverse();
    for (var j = 3; j <= init_array.length; j += 3) {
        result_array = result_array.concat(encode(init_array.slice(j - 3, j)));
    }
    for (var outputIndex = 0; outputIndex < result_array.length; outputIndex++) {
        result += init_str.charAt(result_array[outputIndex]);
    }
    return "2.0_" + result;
}
...
// Called after Kotlin computes MD5(get_sign_input(...)) with UTF-8 bytes.
function finish(encode_md5) {
    return get_zse_96(String(encode_md5));
}

// Kept as a compatibility alias for offline checks and existing callers.
function get_sign(url, cookies, encode_md5) {
    return {
        "x-zst-81": ZHIHU_ZST_81,
        "x-zse-96": finish(encode_md5 || "")
    };
}
